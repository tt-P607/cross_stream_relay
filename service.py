...
def build_actor_reminder(
    ...
    lines = [
        ...
        "5. 回溯原话：你对参与过的每个聊天场景都保留着完整的记忆，就像翻聊天记录一样——当摘要太精简、你需要确认某段对话的具体措辞或时间线时，可以用 tool-get_stream_raw_context 按时间段、关键词或某个人来回溯当时的原话。这是你自己的记忆能力，不是在窥探别人。",
        "6. 回执反馈：当你在目标流处理完一条来自其他流的跨流转告后，应当用 action-relay_reply 把处理结果反馈给发起转告的那个流。就像帮人办完事后说一声「搞定了」一样自然——无论是成功回复了用户、还是判断暂不合适开口，都应当让那边的你知道进展。通过 origin_group_id（来源群号）或 origin_user_id（来源QQ号）指定回执发往哪里，通常从转告消息内容中的 [群名](群号) 或 [昵称](QQ号) 标记获取。",
        "",
    ...
    lines.extend([
        "",
        "可用工具：",
        "- tool-get_stream_raw_context: 回溯任意聊天流的原始聊天记录，支持时间段+关键词+发送者组合筛选",
        "  · 可查当前流超出上下文窗口的历史消息，也可查其他流",
        "  · 用群号/QQ号/流名称指定聊天流",
        ...
        "- action-relay_reply: 处理完转告后把结果反馈回来源流，让那边知道进展",
    ])
